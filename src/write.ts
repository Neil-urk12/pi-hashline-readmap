...
const WRITE_PROMPT = readFileSync(
	new URL("../prompts/write.md", import.meta.url),
	"utf-8",
).trim();
...
export async function executeWrite(opts: {
	...
	try {
		existedBeforeWrite = existsSync(filePath);
		if (existedBeforeWrite) {
			const stat = statSync(filePath);
			if (stat.size <= MAX_INLINE_DIFF_CONTENT_CHARS)
	...
	if (looksLikeBinary(Buffer.from(content, "utf-8"))) {
		...
		ptcWarnings.push(
			buildPtcWarning("binary-content", "File content appears to be binary."),
		);
		return {
	...
	if (existedBeforeWrite && previousContent !== null) {
...
export function registerWriteTool(
	...
	const tool = {
		...
		parameters: Type.Object({
			...
			map: Type.Optional(
				Type.Boolean({ description: "Append structural map to output" }),
			),
		}),
		async execute(
			_toolCallId: string,
			...
			const binaryWarning = result.ptcValue.warnings.find(
				(w) => w.code === "binary-content",
			);
			if (binaryWarning) {
				return {
		...
		renderResult(result: any, _options: any, theme: any, ...rest: any[]) {
			const context: {
				expanded?: boolean;
			...
			const inlineDiff = result.details?.inlineDiff as
				| InlineDiffMetadata
				| undefined;
			if (inlineDiff) {
				const component = context.lastComponent ?? new Text("", 0, 0);
				component.setText(
					renderInlineDiffMetadata(
						inlineDiff,
						theme,
						context,
