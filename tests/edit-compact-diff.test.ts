...
import { buildAllDiffs } from "../src/diff-builder.js";
...
describe("edit compact diffs", () => {
	...
	it("single-line change produces compact format: LINE:HASH|old → LINE:HASH|new", () => {
		...
		expect(r.diff).toMatch(/^\d+:[0-9a-f]{3}\|line two → \d+:[0-9a-f]{3}\|line TWO$/);
	...
	it("single-line deletion produces compact format", () => {
		...
		const newContent = "line one\nline three";
		const r = buildAllDiffs(oldContent, newContent, "sample.ts");
		expect(r.diff).toMatch(/^\d+:[0-9a-f]{3}\|line two → \[deleted\]$/);
		expect(r.firstChangedLine).toBe(2);
	});
	...
	it("identical content returns empty diff", () => {
		...
		const r = buildAllDiffs(content, content, "sample.ts");
		expect(r.diff).toBe("");
